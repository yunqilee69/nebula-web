...
import { getBuiltInLoginMethods, mergeLoginBadges } from '@/auth/auth-methods';
import type {
  AuthInitResp,
    ...
    <Flex vertical gap={token.marginMD}>
      ...
      {alternateMethods.length > 0 && (
        <Flex vertical align="center" gap={token.marginXS}>
          ...
          <Space size={[token.marginXS, token.marginXS]} wrap style={{ justifyContent: 'center' }}>
...
function WechatQrPanel({ authService, onSuccess }: WechatQrPanelProps) {
  ...
  return (
    <Flex vertical align="center" gap={8}>
      {trustedQrCodeUrl ? (
        <img src={trustedQrCodeUrl} alt="微信登录二维码" style={{ maxWidth: 200 }} />
      ) : (
