...
import { Avatar, Button, Card, Descriptions, Empty, Form, Input, Popconfirm, Space, Table, Tag, theme as antdTheme } from 'antd';
    ...
    <PageContainer>
      <Space orientation="vertical" size={token.marginMD} style={{ width: '100%' }}>
        <Card title={t('auth.profileInfo.sections.basic')} loading={profileLoading} extra={<Button icon={<ReloadOutlined />} onClick={() => void loadProfile()}>{t('auth.profileInfo.actions.refresh')}</Button>}>
          <Space orientation="vertical" size={token.marginMD} style={{ width: '100%' }}>
            <Space align="center" size={token.marginMD}>
              <Avatar src={profile?.avatar} size={64} style={{ background: token.colorPrimary }}>
        ...
        <Card title={t('auth.profileInfo.sections.oauth2')} loading={bindingsLoading} extra={<Button icon={<ReloadOutlined />} onClick={() => void loadBindings()}>{t('auth.profileInfo.actions.refresh')}</Button>}>
          {bindings.length > 0 ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: token.marginMD }}>
        ...
        <Card title={t('auth.profileInfo.sections.loginRecords')}>
          <NeTable<LoginRecordResp>
            rowKey="id"
            request={async ({ current, pageSize }) => {
              const result = await service.pageLoginRecords({ pageNum: current, pageSize });
          ...
          >
            <Table.Column<LoginRecordResp> title={t('auth.profileInfo.columns.loginType')} dataIndex="loginType" key="loginType" render={(value: string | undefined) => renderNotProvided(value)} />
            <Table.Column<LoginRecordResp> title={t('auth.profileInfo.columns.loginIp')} key="loginIp" render={(_, record) => renderNotProvided(resolveLoginIp(record))} />
            <Table.Column<LoginRecordResp> title={t('auth.profileInfo.columns.userAgent')} dataIndex="userAgent" key="userAgent" render={(value: string | undefined) => renderNotProvided(value)} />
            <Table.Column<LoginRecordResp> title={t('auth.profileInfo.columns.loginTime')} dataIndex="loginTime" key="loginTime" render={(value: string | undefined) => renderNotProvided(value)} />
            <Table.Column<LoginRecordResp>
            ...
            />
            <Table.Column<LoginRecordResp> title={t('auth.profileInfo.columns.failReason')} dataIndex="failReason" key="failReason" render={(value: string | undefined) => renderNotProvided(value)} />
          </NeTable>
        </Card>
      </Space>
    </PageContainer>
